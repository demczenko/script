const ids = [
  {
    main_id: "422441",
    src_origin: "ttps://beliani.info/newsletter/2022/",
    src_name: "240219Category11.png",
    src: "https://beliani.info/newsletter/2022/240219Category11.png",
  },
  {
    main_id: "440578",
    src_origin: "https://beliani.info/newsletter/2022/",
    src_name: "240219Category12.png",
...
let counter = 0;
async function getProductData(product, counter) {
  ...
  const prices = await getPrices(
    slaves_ids.filter((slave) => slave.username !== "Beliani RO")
  );
          ...
          [`Product src ${counter}`]:
            "src" in product
              ? typeof product.src === "function"
                ...
                : product.src
              : null,
          [`Product name ${counter}`]: name,
          main_id: product.main_id,
          slug: countryLink,
          [`Product href ${counter}`]: href,
          [`Product low ${counter}`]: lowPrice,
          [`Product high ${counter}`]: highPrice,
        });
      }
    }
  }
...
const ids_response = await Promise.allSettled(
  ids.map((product) => {
    counter++;
    return getProductData(product, counter);
  })
);

const data = ids_response
  .map((item) => {
...
const group = data.reduce((acc, elem) => {
  const isSlugIexist = acc.find((obj) => obj.slug === elem.slug);
  if (isSlugIexist) {
    acc = acc.map((item) => {
      if (item.slug === isSlugIexist.slug) {
        const new_item = {
          ...item,
          ...elem,
        };
